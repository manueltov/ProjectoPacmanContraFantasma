...
class ClassicGameRules:
    ...
    def getMaxTotalTime(self, agentIndex):
        """ O MaxTotalTime é igual ao timeout para todos"""
        return self.timeout
    ...
    def getMoveTimeout(self, agentIndex):
        """ o time out não depende do index do jogador-igual para todos"""
        return self.timeout
...
def readCommand(argv):
    ...
    parser.add_option('-b', '--ghostArgs', dest='ghostArgs',
                      help='Comma separated values sent to ghosts. e.g. "opt1=val1,opt2,opt3=val3"')
    parser.add_option('-x', '--numTraining', dest='numTraining', type='int',
                      help=default('How many episodes are training (suppresses output)'), default=0)
    parser.add_option('--frameTime', dest='frameTime', type='float',
                      help=default('Time to delay between frames; <0 means keyboard'), default=0.1)
    parser.add_option('-y', '--limMoves', dest='limMoves', type='int',
                      help=default('The limit of double moves (1 move means Pac and Ghost)'), default=10000)
    ...
    args['limMoves'] = options.limMoves

    # Special case: recorded games don't use the runGames method or args structure
    if options.gameToReplay != None:
        print('Replaying recorded game %s.' % options.gameToReplay)
        import pickle
        f = open(options.gameToReplay,'br')
        try:
...
def runGames(layout, pacman, ghosts, display, numGames, record, limMoves,numTraining=0, catchExceptions=False, timeout=30):
    import __main__
    ...
    for i in range(numGames):
        ...
        if record:
            ...
            ffname = ('./results/recorded-game-'+namePac+nameGhost+"-"+str(i+1)+
                '-').join([str(t) for t in time.localtime()[1:6]])
            #print(fname)
            f = open(fname, 'bw')
